...
$t_max = $_REQUEST[n]; 
if (!t_max || $t_max<1 || $t_max>9) $t_max=9; 
...
$fp1 = fsockopen($host, $port, $errno, $errstr, 1); //open connection
if(!$fp1) { //if this fails, I'm done.... 
echo '<div id="strerr">Houston we have a problem?<br>';
echo 'Try refresh the list a bit later...</div>';
fclose($fp1); 
$success++; 
} else { 
...
$output_string .= $r . '. ' .  ucwords(str_replace('ë','&#235;',str_replace('_',' ',str_replace("'", "'",str_replace('"', '"',$track[$r]))))) . '<br>'; 
